...
**Note**: When initializing the TAVIE location-scale model under custom location-scale error distribution family, the user is required to pass callable functions `afunc` and `cfunc` to the `TAVIE_loc_scale()` class. More details are to follow in the subsequent section.
...
# the results are stored in 'mse_beta_TAVIE', 'mse_tau2_TAVIE',
# 'mse_beta_MFVI', 'mse_tau2_MFVI', 'mse_beta_BBVI', 'mse_tau2_BBVI',
# 'time_TAVIE', 'time_MFVI', and 'time_BBVI'.
```
...
<p align="center">
  <img src="assets/TAVIE_MFVI_BBVI_MSE_Runtime_Comparison.gif" alt="TAVIE Laplace ELBO" width="800"/>
...
**While flexible VI algorithms can provide good approximations, they incur greater runtime and come with no theoretical guarantees!** In all the above numerics conducted, we showcase stronger empirical results for TAVIE when compared to the different BBVI algorithms performed. The comparison of TAVIE (estimates, runtime, and convergence diagnostics) against various BBVI algorithms and MFVI module under a different Student's-t SSG likelihood generation is presented below.
...
<p align="center">
  <img src="assets/comparison_QR_animation.gif" alt="comparison QR" width="900"/>
